...
if ( CLIENT ) then
    language.Add( "Tool.coolant_tank.name", "Coolant Tank" );
    language.Add( "Tool.coolant_tank.desc", "Coolant Storage" );
	language.Add( "Tool.coolant_tank.0", "Left Click to place tank" );
end

local entityModel = "models/coolant_tank.mdl"
...
if ( not trace.HitPos ) then return false; end
	...
	local createdEntity = ents.Create( "coolant_tank" )
	...
	undo.Create( "coolant_tank" );
		undo.AddEntity( createdEntity );
		undo.AddEntity( const );
		undo.SetPlayer( ply );
		undo.SetCustomUndoText("Undone Coolant Tank")
	undo.Finish();

	ply:AddCleanup( "coolant tanks", createdEntity );
...
function TOOL:UpdateGhost( ent, player )
	...
	if ( not trace.Hit or trace.Entity:IsPlayer() or trace.Entity:GetClass() == "coolant_tank" ) then
		ent:SetNoDraw( true );
		return;
	end
...
function TOOL.BuildCPanel( panel )
	...
	panel:AddControl("CheckBox", {
	    ...
	    Command = "coolant_tank_weldToTarget"
	})
