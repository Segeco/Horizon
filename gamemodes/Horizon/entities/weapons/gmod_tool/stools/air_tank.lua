...
if ( CLIENT ) then
    language.Add( "Tool.air_tank.name", "Air Tank" );
    language.Add( "Tool.air_tank.desc", "Air Storage" );
	language.Add( "Tool.air_tank.0", "Left Click to place tank" );
end

local entityModel = "models/air_tank.mdl"
...
if ( not trace.HitPos ) then return false; end
	...
	local createdEntity = ents.Create( "air_tank" )
...
function TOOL:UpdateGhost( ent, player )
	...
	if ( not trace.Hit or trace.Entity:IsPlayer() or trace.Entity:GetClass() == "air_tank" ) then
		ent:SetNoDraw( true );
		return;
	end
