...
if ( CLIENT ) then
    language.Add( "Tool.mining_drill.name", "Mining Drill" );
    language.Add( "Tool.mining_drill.desc", "Planetary Mining" );
	language.Add( "Tool.mining_drill.0", "Left Click to place drill" );
end

local entityModel = "models/mining_drill.mdl"
...
if ( not trace.HitPos ) then return false; end
	...
	local createdEntity = ents.Create( "mining_drill" )
	...
	undo.Create( "mining_drill" );
		undo.AddEntity( createdEntity );
		undo.AddEntity( const );
		undo.SetPlayer( ply );
		undo.SetCustomUndoText("Undone Mining Drill")
	undo.Finish();

	ply:AddCleanup( "mining drills", createdEntity );
...
function TOOL:UpdateGhost( ent, player )
	...
	if ( not trace.Hit or trace.Entity:IsPlayer() or trace.Entity:GetClass() == "mining_drill" ) then
		ent:SetNoDraw( true );
		return;
	end
...
function TOOL.BuildCPanel( panel )
	...
	panel:AddControl("CheckBox", {
	    ...
	    Command = "mining_drill_weldToTarget"
	})
