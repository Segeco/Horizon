...
if ( CLIENT ) then
    language.Add( "Tool.coolant_compressor.name", "Coolant Compressor" );
    language.Add( "Tool.coolant_compressor.desc", "Extract Coolant from habitable planets" );
	language.Add( "Tool.coolant_compressor.0", "Left Click to place compressor" );
end

local entityModel = "models/coolant_compressor.mdl"
...
if ( not trace.HitPos ) then return false; end
	...
	local createdEntity = ents.Create( "coolant_compressor" )
	...
	undo.Create( "coolant_compressor" );
...
function TOOL:UpdateGhost( ent, player )
	...
	if ( not trace.Hit or trace.Entity:IsPlayer() or trace.Entity:GetClass() == "coolant_compressor" ) then
		ent:SetNoDraw( true );
		return;
	end
...
function TOOL.BuildCPanel( panel )
	...
	panel:AddControl("CheckBox", {
	    ...
	    Command = "coolant_compressor_weldToTarget"
	})
