...
if ( CLIENT ) then
    language.Add( "Tool.hydrogen_tank.name", "Hydrogen Tank" );
    language.Add( "Tool.hydrogen_tank.desc", "Hydrogen Storage" );
	language.Add( "Tool.hydrogen_tank.0", "Left Click to place tank" );
end

local entityModel = "models/hydrogen_tank.mdl"
...
if ( not trace.HitPos ) then return false; end
	...
	local createdEntity = ents.Create( "hydrogen_tank" )
	...
	undo.Create( "hydrogen_tank" );
		undo.AddEntity( createdEntity );
		undo.AddEntity( const );
		undo.SetPlayer( ply );
		undo.SetCustomUndoText("Undone Hydrogen Tank")
	undo.Finish();

	ply:AddCleanup( "hydrogen tanks", createdEntity );
...
function TOOL:UpdateGhost( ent, player )
	...
	if ( not trace.Hit or trace.Entity:IsPlayer() or trace.Entity:GetClass() == "hydrogen_tank" ) then
		ent:SetNoDraw( true );
		return;
	end
...
function TOOL.BuildCPanel( panel )
	...
	panel:AddControl("CheckBox", {
	    ...
	    Command = "hydrogen_tank_weldToTarget"
	})
