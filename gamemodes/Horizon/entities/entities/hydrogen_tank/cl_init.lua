...
net.Receive( "netHydrogenTank", function()
	local entity = net.ReadEntity()	
	entity.dispHydrogen = math.Round( net.ReadFloat() )
	entity.dispMaxHydrogen = math.Round( net.ReadFloat() )
...
function DrawInfo()
	...
	if ent.dispMaxHydrogen == nil then ent.dispMaxHydrogen = 0 end

	if tr.Entity:IsValid() and tr.Entity:GetPos():Distance(LocalPlayer():GetPos()) < 150 then
	 	if tr.Entity:GetClass() == "hydrogen_tank" then
	 		local text = ent.PrintName .. 
	 					 "\nHydrogen: " .. commas( ent.dispHydrogen ) .. "/" .. commas( ent.dispMaxHydrogen )
...
hook.Add( "HUDPaint", "DrawInfoHydroTank", DrawInfo )